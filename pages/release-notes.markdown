...
* Public: Buy tickets — a signed-in buyer builds a cart on the event's Tickets tab and pays in Stripe Checkout; the order page under the event and "My tickets" on the account hold one QR per ticket, the ticket email arrives in the buyer's language, and a refund made in Stripe is mirrored onto the order (see [Buying tickets](/spectator/buying-tickets/))
* Check-in: Vote4Dance ID — every account has a QR card with photo, name, birth date, nationality and club, opened from the account page on the website and from the Profile tab in the app; the desk scans it to open the person's screen (see [Your Vote4Dance ID](/dancer/vote4dance-id/))
* Check-in: Person screen — one screen per scanned person with their registrations, their tickets (use / undo) and an Arrived button recorded per person and day, separately from tickets used and from the club check-in; the lookup box takes a scanned ID, a ticket code, or a name or email, and a Scan button opens the camera inside the app (see [At the door](/manager-guide/the-door/))
...
* School/Club: The licence page shows each member's name and birth date and warns when a roster member has none; the permissions guide describes what each role may actually do
* Public: The Registration tab says when registration opens instead of showing a close date in the future
* Public: "Hide results from position" applies to every row of a round without placements (star collection, point summary), not only the placed ones
* Public: The Kontrollant tile shows for users who have the Supervisor app
* Dancer app: A results class with nothing published shows that instead of a blank sheet, the schedule tab is localized, competition dates no longer read "Invalid Date", the Android launch screen shows the logo at its intended size, and the image picker no longer offers HEIC and states the correct size limit
## 2026-09-01
 ⭐ **New**
