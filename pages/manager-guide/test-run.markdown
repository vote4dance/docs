...
2. For the federation question, choose **standalone** or **test** rather than a federation, so
   no license or ranking rules get in the way.
3. Choose the **practice** setup rather than billing.
4. Enable **bootstrap**. It generates a working competition for you: a main floor, a judging
   panel, a default schedule, a default screen configuration, practice classes, generated
   rounds and random teams.

You now have something you can run end to end within minutes. Practice competitions are
deliberately limited and are excluded from federation reporting, so nothing you do here shows
up anywhere real.

Name it so nobody mistakes it for the real thing, for example
...
4. Set a judge PIN for the practice event and write it down.
...
- The printed station QR codes and the judge PIN
...
7. Note what you did: which input, which resolution, which zoom. You will want it on the day.

### C. Devices and people (15 min)

1. Judges' tablets: open the Judging app, unlock with the judge PIN, leave them on the
   judges' table.
...
| **Judging** | Every judge tablet unlocks with the PIN and their marks land in Manager. |
...
1. Keep the practice event. It stays useful for training new functionaries.
...
3. Turn your notes into a one-page run sheet for the morning: order of setup, who does what,
   the judge PIN, and the venue contact's phone number.
...
2. Display computer to the projector; Screens station, floor, fullscreen, zoom as noted (10 min).
3. Judges' tablets out, PIN entered (5 min).
...
- Missing judge PIN, or judges who were never added to the panel for a class.
