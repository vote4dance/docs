...
| 4 | Floor | `Manager → Event → Floor` | A round can be placed on a floor, so the Speaker, the Supervisor and each screen can follow one floor. A new event already has a **Main floor**, which is enough for most competitions. See [Floors](/manager-guide/floors/). |
...
| 7 | Schedule | `Manager → Event → Schedule` | Each round sits on a schedule item, which gives it a time and an order and feeds the public schedule, the speaker and the screens. A dated day header is also what the dressing-room and admission-per-day features need. See [Schedule and conflicts](/manager-guide/schedule/). |
...
Before the day, open [**Problems**](/manager-guide/validator/) in Manager and resolve everything it flags. Problems runs a
set of automatic tests over your setup — start-number conflicts, participants scheduled in two
overlapping rounds, advancement that does not add up, and (for DSF competitions) license
conflicts — and re-runs them whenever you change something.
